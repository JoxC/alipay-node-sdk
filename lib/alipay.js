...
function Alipay(opts) {
    ...
    this.rsaMerchantPublic = opts.rsaMerchantPublic && fs.readFileSync(opts.rsaMerchantPublic, 'utf-8');
}
 ...
 */
props.signVerify = function (response, publicKey) {
    ...
    if(res) {
        return utl.signVerify(JSON.stringify(res), sign, publicKey || this.rsaPublic, this.signType);
    } else {
        var tmp = utl.encodeParams(ret);
        return utl.signVerify(tmp.unencode, sign, publicKey || this.rsaPublic, this.signType);
    }
}
